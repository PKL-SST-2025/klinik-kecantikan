...
import api from '../api/api'; // Pastikan Anda membuat file ini untuk mengelola base URL
// Anda perlu membuat file `api.ts` di folder `src/api` untuk mengatur base URL API
...
const AppointmentSchedulePage: Component = () => {
    ...
    const [filterDokterId, setFilterDokterId] = createSignal<string | 'all'>('all'); // Dokter ID mungkin UUID
    ...
    const navigateToPatientDetail = (appointment: Appointment) => {
        navigate(`/pasien?id=${appointment.pasien_id}`);
    };
    ...
    // --- Helper Functions ---
    // Sesuaikan dengan struktur properti dari backend (misalnya pasien.nama_lengkap)
    ...
    const getAppointmentDetails = (appointment: Appointment) => {
        const pasien = pasienList().find(p => p.id === appointment.pasien_id); // sesuaikan dengan nama properti di BE
        const dokter = dokterList().find(d => d.id === appointment.dokter_id); // sesuaikan dengan nama properti di BE
        const treatments = treatmentList().filter(t => appointment.treatment_ids.includes(t.id)); // sesuaikan dengan nama properti di BE
    ...
    const updateAppointmentStatus = async (
        id: string, // ID dari backend kemungkinan string (UUID)
        ...
        try {
            ...
            // Perbarui state lokal dengan data yang diperbarui dari server
                                ...
                                return (
                                    <div class={`bg-white/80 backdrop-blur-lg rounded-3xl border border-white/20 shadow-xl p-6 border-l-4 border-l-${statusConfig.color}-500`}>
                                        ...
                                        <div class="pt-4 border-t flex items-center flex-wrap gap-3">
                                            ...
                                            {/* ... (Buttons untuk completed, cancelled, dsb. tetap sama) */}
